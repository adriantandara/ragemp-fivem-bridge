interface PluginConfig {
  side: string;
  scriptKey: string;
  resourceStartEvent?: string;
  contextExtras?: (name: string, resource: string) => Record<string, unknown>;
}
...
interface BuiltinModule {
  name: string;
  default: (api: { mp: unknown; plugin: PluginContext }) => void;
}

export class PluginManager {
  _plugins: Map<string, PluginContext> = new Map();
  _builtins: BuiltinModule[] = [];
  _side: string;
  _scriptKey: string;
  _resourceStartEvent: string | undefined;
  _contextExtras: (name: string, resource: string) => Record<string, unknown>;
  _disabled: Set<string> | undefined;
  ...
  registerBuiltin(setupModule: BuiltinModule): void {
  ...
  loadAll(): void {
    ...
    if (this._resourceStartEvent) {
      on(this._resourceStartEvent, (resource: string) => this._tryLoadResource(resource));
    }
  }

  _readDisabled(): Set<string> {
    const disabled = new Set<string>();
    const readFrom = (res: string): void => {
      if (!res) return;
  ...
  _tryLoadResource(resource: string): void {
    ...
    try {
      const fn = new Function("mp", "plugin", code) as (mp: unknown, plugin: PluginContext) => void;
      this._invoke(name, resource, (api: { mp: unknown; plugin: PluginContext }) => fn(api.mp, api.plugin), { builtin: false });
    } catch (err) {
      console.error(`[bridge:plugins] Failed to load '${name}' from '${resource}':`, err);
    }
  }

  _invoke(name: string, resource: string, setupFn: (api: { mp: unknown; plugin: PluginContext }) => void, meta: PluginMeta): void {
    const mp = (globalThis as Record<string, unknown>).mp;
  ...
  _makeContext(name: string, resource: string, builtin: boolean): PluginContext {
    const base: PluginContext = {
      name,
      resource,
      builtin,
      namespace(eventName: string): string { return `${name}:${eventName}`; },
      log(...args: unknown[]): void { console.log(`[plugin:${name}]`, ...args); },
    };
    return Object.assign(base, this._contextExtras(name, resource) ?? {});
  }

  list(): PluginContext[] { return [...this._plugins.values()]; }
  get(name: string): PluginContext | null { return this._plugins.get(name) ?? null; }
  has(name: string): boolean { return this._plugins.has(name); }
}
