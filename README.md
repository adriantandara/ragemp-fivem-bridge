...
<script defer src="./_bridge.js"></script>
...
### React error #299 — `createRoot` container is null

**Symptom:** `Uncaught Error: Minified React error #299` in the NUI console.

**Cause:** The app script runs before the DOM is parsed. Without `type="module"`, scripts in `<head>` are no longer automatically deferred.

**Fix:** Add `defer` to all `<script>` tags and ensure `env.js` (if present) loads before the app:
...
### `playerReady` not firing on the client

**Symptom:** Server-side `playerReady` fires correctly but client-side handlers registered with `mp.events.add("playerReady", ...)` never fire.

**Cause:** `playerReady` is a client-side internal event. The bridge triggers it by echoing `ragemp:playerReady` from the server back to the client after processing the initial client → server signal. This round-trip ensures all client scripts have registered their handlers before the event fires.

**No fix needed** — this is built into the bridge. If you are seeing the event not fire, check that `@ragemp-fivem-bridge/server.js` is loaded before your gamemode in `server_scripts`.

---

### `Unable to resolve PlayerMp` warnings on `playerReady`

**Symptom:**
```
Event rejected: Unable to resolve PlayerMp for parameter at index 0
```

**Cause:** The bridge resource's own `client.js`/`server.js` were running in the bridge's isolated resource context (not the gamemode context), causing a double emit of `ragemp:playerReady`.

**Fix:** The bridge's `fxmanifest.lua` must declare `server_script` and `client_script` (required for `@`-import compatibility), but the scripts guard against self-initialisation:
...
The scripts skip all initialisation when `GetCurrentResourceName() === "ragemp-fivem-bridge"`. Ensure you are on bridge version ≥ 0.1.0.

---

### Filesystem permission errors on server startup

**Symptom:**
```
Filesystem permission check from 'ragemp' for permission fs.write — no device found
```

**Cause:** Node.js logging libraries resolve `logs/` relative to `process.cwd()`, which in FiveM points to the server root — a path the resource sandbox blocks.

**Fix:** The bridge's `fs-compat` plugin overrides `process.cwd()` to return the resource path, redirecting writes to a location the resource is allowed to write. This is automatic — no configuration needed.

---

### `source` vs `globalThis.source` in FiveM Node.js

When writing bridge plugins or custom server code, always use the bare `source` global (not `globalThis.source`) inside `on`/`onNet` callbacks. FiveM exposes `source` through the scope chain, not always as a property of `globalThis`. Using `globalThis.source` can result in players being stored in the pool with `id = undefined`, breaking all subsequent player lookups.

```js
on("playerConnecting", (name) => {
  const player = new PlayerMp(source);  // correct
});
```
